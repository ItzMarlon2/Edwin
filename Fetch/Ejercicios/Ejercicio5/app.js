...
const arreglo = animales.filter((ani )=> ani.color!="Blanco")
console.log(arreglo)
const crearCard = arreglo =>{
    arreglo.forEach(arre=>{
        template.querySelector('h2').textContent=arre.name
        template.querySelectorAll('p')[0].textContent=arre.type
        template.querySelectorAll('p')[1].textContent=arre.age
        template.querySelectorAll('p')[2].textContent=arre.color
